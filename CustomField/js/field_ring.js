...
CustomFields.FieldRing = function (
  L1,
  L2,
  L3,
  L4,
  ...
  CustomFields.FieldRing.superClass_.constructor.call(
    this,
    value,
    opt_validator,
  );
...
CustomFields.FieldRing.prototype.DEFAULT_VALUE =
  CustomFields.FieldRing.COLOURS[0];
...
CustomFields.FieldRing.prototype.createWidgetView = function () {
  ...
  for (var i = 0; i < 12; i++) {
    ...
    var angle = 30 * i;
    var x =
      (this.RING_WIDTH / 2) * Math.cos(Blockly.utils.math.toRadians(angle)) +
      6.6;
    var y =
      (this.RING_WIDTH / 2) * Math.sin(Blockly.utils.math.toRadians(angle)) + 6;
    ...
    this.editorListeners_.push(
      Blockly.bindEvent_(led, this.getMouseUp(), this, this.onRingLedClick),
    );
...
CustomFields.FieldRing.prototype.createColorSelect = function () {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    colorButton.style.backgroundColor =
      CustomFields.FieldRing.COLOURS[3 + i * 7];
...
CustomFields.FieldRing.prototype.createView_ = function () {
  this.ringGroup_ = Blockly.utils.dom.createSvgElement(
    'g',
    {
  ...
  for (var i = 0; i < 12; i++) {
    ...
    if (!this.ringCell_[i]) {
      this.ringCell_[i] = Blockly.utils.dom.createSvgElement(
        'rect',
        {
          class: 'cellSvg',
          x: x,
